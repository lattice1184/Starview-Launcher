...
- **插件系统（v1.1.7）**：第三方扩展（.dll 实现 `IStarviewPlugin` 接口），左侧导航新增「插件」管理页——导入 / 沙箱试运行 / 停用 / 删除 + 总开关
- **先跑给你看，再决定留不留**：每个插件可「沙箱试运行」——在独立子进程里跑它的初始化，临时目录重定向到一次性沙盒 + 监听敏感目录（桌面/文档/下载/启动器数据）写入 + 超时强杀，报告它到底写了什么、有没有越界、有没有崩溃挂起
- **防投毒**：插件 SHA1 登记（首次导入记录，二次比对不一致 = 掉包 → 拒绝加载）；白名单 API（插件只能拿到受限上下文，不碰裸系统能力）；停用/删除的即时卸载 + 文件被占用时墓碑式「重启后自动清除」防复活
- 开发者：见 `examples/plugins/`（示例插件 + 沙箱探针），`IStarviewPlugin` / `PluginContext`（Log / SettingsDir / Subscribe 事件）白名单接口
- 诚实边界：Windows 试运行为行为监测（非硬文件系统隔离），越界写监测范围外的绝对路径可能不捕获；恶意插件只有防掉包 + 试运行报告，防不了「你主动装的恶意代码」
...
- 自动化测试 890+ 项（下载引擎、登录、JSON 解析、版本匹配、模组兼容修复、更新检查、插件登记/试运行等）
...
- **v1.1.15 2 项修复**：CurseForge 下载——镜像前缀现在对 mediafilez / media 主机也生效（此前只认旧的 edge 域名，配了 CF 镜像也白配）；单个下载源的文件不再被慢速误杀（判死本意是换源、单源没得换，慢也硬啃到底不报错）
- **v1.1.14 6 项修复**：下载稳定性——改设置不再打断正在进行的下载（此前下载中动一下设置，连接池重建导致下载失败、游戏启动即退）；Mac 自动下载 Java 显示进度（不再停在「检测 Java」像卡死）+ 残缺运行时自动补齐；设置页新增「下载 Java」（可选 25/21/17/8，带进度）；Mac 更新覆盖修复（更新脚本适配 .app 应用包，此前「重启安装」覆盖不了、静默失败；失败会提示原因）；联机功能 Mac 修复（EasyTier/Terracotta 的 macOS 哈希补齐，能下载安装了）；错误报告带上游戏日志（latest.log / crash-reports / JVM 崩溃文件）
- **v1.1.13 4 项修复**：Mac 一键补齐 Java——新版要 Java 25、老版本要 Java 17，检测到缺就自动下载官方运行时，不用自己装；Mac 装了 Java 却识别不到的问题修复（.jdk 应用包、java_home、Homebrew 装的都能扫到）；老版本下载「文件缺失」修复（1.8.x/1.12.x 的 natives 库不再误报缺文件，下载能正常完成）；加载器选择提速（多源竞速 + 15 秒上限 + 点版本行后台预热，冷缓存也秒出）
- **v1.1.11 7 项修复**：Mac 打不开根治（完整应用包 + Intel/Apple Silicon 自动匹配）；下载提速（库多连接、慢源 30s→10s 判死、NeoForge 加镜像、bmclapi 每连接限速分片叠加——5000 资源阶段明显加速）；加载器版本列表多源竞速 + 10s 短超时（冷缓存不再卡）；数据目录改 Starview（仅新装默认）；卸载重装弹窗只弹一次、跳过也记住；界面密度（紧凑/舒适）移除；导出整合包后窗口不再变回实色
- **v1.1.10 更新后弹窗**：升级后首次启动弹出「本次更新内容」窗口，列出这个版本修了什么（首装不弹、看过不重弹）；关于页更新日志改为按版本分组展示
- **v1.1.9 Linux 自动更新修复**：更新后文件损坏打不开的根因——解压器对 gzip 压缩包两次读取失效导致内容错位，已改单遍解压 + 更新脚本改为原子替换（失败自动回滚旧版，不再出现更新后目录空/打不开，失败日志可查）；后台自动检查新版本后会直接出现在「关于-重启安装」（不用再手动点检查），发布新版后最多 1 小时内自动推送
- **v1.1.8** 插件导航修复（Linux 下点插件高亮不再卡在上一格、左边竖杠消失）；新装/清空账号后自动创建离线账号「Player」，启动不再被「未登录」拦住，头像旁直接显示名字和账号类型，点击即可管理账号；整合包导入显示真实大小（550MB 级整包不再只显示 8MB，解压进度按实际字节推进）
- **v1.1.7 插件系统**：左侧导航新增「插件」页——导入 / 沙箱试运行 / 停用 / 删除 / 总开关。**沙箱试运行**在独立子进程跑插件初始化，临时目录重定向 + 敏感目录监听 + 超时强杀，报告它写了什么、有没有越界/崩溃/挂起；SHA1 登记防掉包，白名单 API，删除被占用文件时墓碑式「重启后自动清除」防复活。开发者接口见 `examples/plugins/`。自动化测试 836 项全绿
- **v1.1.6** 沙盒启动模式：把游戏关进隔离环境跑——Linux 用 bubblewrap 限制文件读写、macOS 用 sandbox-exec、Windows 严格隔离断网走临时防火墙规则（退出自动清理，断网需管理员权限）；主页启动旁可直接选「普通启动/保护模式/严格隔离」，设置页也能设默认；装模组查前置更快（依赖解析两阶段并行）；修复版本删除后又冒出来（版本页算体积不再写盘）、整合包下载没有大小和进度（大小未知时自动回落服务器返回的真实长度）
- **v1.1.5** 后台静默更新（Windows/Linux/Mac 全平台，启动后自动查新版，下好提示重启生效）；运行中一键「强制关闭」直接整树强杀游戏进程；启动进度条补齐（能看见「拉起游戏窗口/游戏加载中」），控制台日志带时间戳和颜色分级（警告/错误/致命分色）；模组搜索卡片去图纯文字分块 + 每卡显示支持版本 + 版本筛选；模组图标显示根治（坏缓存自动清理、加载失败不再没动静）；依赖解析并行化，装模组查前置更快
