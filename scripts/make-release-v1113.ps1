...
#
# 8-31 修复：tar.gz 保留原名（starview-*-YYYYMMDD.tar.gz）——更新器 MatchPlatformAsset
# 按 "starview-osx-arm64-"/"starview-osx-x64-"/"starview-linux-x64-" 前缀匹配，改名成
# Starview-Launcher-* 会让 Mac/Linux 静默更新找不到资产（v1.1.11 的 bug）。exe 仍改名
# （更新器对 Windows 精确匹配 "Starview-Launcher.exe"）。
...
if ($exists) { Write-Host "release $tag already exists (id=$($exists.id))" }
  ...
  $body = [IO.File]::ReadAllText((Join-Path $root "..\body-v1113.md"), [Text.Encoding]::UTF8)
  $json = @{ tag_name = $tag; name = "Starview Launcher v1.1.13"; body = $body; draft = $false; prerelease = $false } | ConvertTo-Json -Depth 3
...
$dest = "C:\Users\yanka\Desktop\Starview发布\v1.1.13"
