# v1.1.14 release 创建：建 release + 上传 5 个资产（win 2 exe + linux/osx 3 tar.gz）
# ASCII-only 脚本（中文 body 从外部 UTF-8 文件读入，规避 PS 5.1 编码坑；
# 中文路径不用字面量，一律 glob 匹配 + 大小/文件名映射）
#
# tar.gz 保留原名（starview-*-YYYYMMDD.tar.gz）——更新器 MatchPlatformAsset 按
# "starview-osx-arm64-"/"starview-osx-x64-"/"starview-linux-x64-" 前缀匹配，改名会让 Mac/Linux
# 静默更新找不到资产（v1.1.11 的 bug，v1.1.13 起已修）。exe 仍改名（Windows 精确匹配 "Starview-Launcher.exe"）。
# 注意：本脚本必须带 UTF-8 BOM（PS 5.1 按 GBK 读会乱掉中文路径 → 资产 0 上传）。
...
if ($exists) { Write-Host "release $tag already exists (id=$($exists.id))" }
  ...
  $body = [IO.File]::ReadAllText((Join-Path $root "..\body-v1114.md"), [Text.Encoding]::UTF8)
  $json = @{ tag_name = $tag; name = "Starview Launcher v1.1.14"; body = $body; draft = $false; prerelease = $false } | ConvertTo-Json -Depth 3
...
$dest = "C:\Users\yanka\Desktop\Starview发布\v1.1.14"
...
# 三平台 tar.gz：保留原名上传（更新器按 starview-{platform}- 前缀匹配，不得改名）
foreach ($t in @(Get-ChildItem $dest -Filter "starview-*.tar.gz" -ErrorAction SilentlyContinue)) {
  $files += @{ Local = $t.FullName; Name = $t.Name }
}
