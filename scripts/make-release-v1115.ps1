# v1.1.15 release 创建：建 release + 上传 5 个资产（win 2 exe + linux/osx 3 tar.gz）
...
if ($exists) { Write-Host "release $tag already exists (id=$($exists.id))" }
  ...
  $body = [IO.File]::ReadAllText((Join-Path $root "..\body-v1115.md"), [Text.Encoding]::UTF8)
  $json = @{ tag_name = $tag; name = "Starview Launcher v1.1.15"; body = $body; draft = $false; prerelease = $false } | ConvertTo-Json -Depth 3
...
$dest = "C:\Users\yanka\Desktop\Starview发布\v1.1.15"
